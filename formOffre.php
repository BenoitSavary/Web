 <div class="margin">
<h2> Formulaire de l'offre de stage </h2>
<div class="separation"></div>

    <form>
      <h3> Veuillez renseigner les informations sur l'intreprise : </h3>
     
<div class="group">
            <label> Nom de l'entreprise* </label>
            <input type="text" />
          </div>
    

      <div class="form">
      

       
        

            <div class="group">
            <label> Début du stage* </label>
            <input type="date" />
          </div>
           <div class="group">
            <label> ville* </label>
            <select >
            <option value="test">--Sélectionner une ville--</option>
          </select>
          </div>

           

           
         <div class="group">
            <label> Complément d'adresse*  </label>
            <input type="email" />
          </div>
          <div class="group">
            <label> Adresse mail du contact* </label>
            <input type="email" />
          </div>
         </div>
 <div class="Des2"> 
      <div class="group20">
        <button>Ajouter des informations sur l'entreprise</button>
      </div>
    </div>
      <div class="separation"></div>
          <h3>Veuillez renseigner les informations sur le stage :</h3>

<div class="group">
            <label> Intitulé de l'offre* </label>
            <input type="text" />
          </div>
          <div class="form">
          
           <div class="group">
            <label> Début du stage* </label>
            <input type="date" />
          </div>
          <div class="group">
            <label> Durée du stage* </label>
            <input type="number" />
          </div>
          <div class="group">
            <label> Compétence principale* </label>
            <select  >
            <option value="test">--Selectionner une majeure-- </option>
            </select>
          </div>
          <div class="group">
            <label> Compétence secondaire* </label>
            <select  >
            <option value="test">--Selectionner une mineure-- </option>
            </select>
          </div>
           <div class="group">
            <label> Gratification* </label>
            <input type="text" />
          </div>
          <div class="group">
            <label> Nombre de place* </label>
            <input type="number" />
          </div>
       
          
          <div class="group">
            <label>Détail de l'offre*</label>
            <textarea placeholder="Saisissez le détail de l'offre ici..." rows="10"></textarea>
          </div>
        </div>
      
      <div class="Des"> 
      <div class="group20">
        <button>Enregistrer</button>
      </div>
      <div class="group20">
        <button>Annuler</button>
      </div>
      </div>
    </form>
  </div>
