...
function getUsers()
{
   ...
   xhr.open("GET", "http://127.0.0.1:8000/dataentreprise", true);
   xhr.onload = function()
   {
   if( xhr.status == 200)
   {
...
function adresses(){
   var html = "";
   var xhr = new XMLHttpRequest();
   xhr.open("GET", "http://127.0.0.1:8000/dataavoir", true);
   xhr.onload = function()
   {
      if( xhr.status == 200)
      {
         ...
         entreprises.forEach(entreprise =>
            {
               if(entreprise.entreprises_id == idEntreprise){
                  ...
                  xhm.open("GET", "http://127.0.0.1:8000/dataadresse", true);
                  xhm.onload = function()
                  {
                     ...
                     ent.forEach(en =>
                     {
                        if(entreprise.adresses_id == en.id){
                           html += "<li>"+en.ville+" "+en.codePostal+" "+en.adresse+"</li>";
                        }
                     });
                  }
               }
